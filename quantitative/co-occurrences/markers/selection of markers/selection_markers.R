...
m_selection_markers <- read.csv("/home/sitel/Téléchargements/cooccurrences/co-occurrences/markers/selection of markers/epist_verbs.csv", header = TRUE, row.names = 1, sep = "\t")
m_selection_markers_final <- 0.5*m_selection_markers

write.csv(m_selection_markers_final,"/home/sitel/Téléchargements/cooccurrences/co-occurrences/markers/selection of markers/epist_verbs_final.csv")

library(vcd)
library(vcdExtra)

#read the dataframe as matrix
new_m_selection_markers_final <- as.matrix(m_selection_markers_final)

#produce sieve
#deleted option "label = TRUE" to show Pearson residuals
sieve(new_m_selection_markers_final, main = "Epistemic adverbs and most frequent verbs", shade = TRUE, gp = NULL, gp_tile = gpar(), labeling = labeling_values, rot_labels = c(top = 90, left = 0), offset_labels = c(0.7, 0, 0, 1.5), scale = 1)
