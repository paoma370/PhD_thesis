...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 31 12:02:34 2022

@author: sitel
"""
############TO EXTRACT ONLY THE SENTENCES WITH ONE MARKER#####################################################################################
...
modal_lemmas_1 = ['debeo','possum','queo','nequeo','decet','licet','oportet','valet','iniquus','aptus','ineptus','certus','incertus',\
	...
	'possibiliter','probabiliter'] #deleted certo and dubio because they are lemmatised as certus and dubius
...
for i in range(len(sentences)): #range to have them as a number: it iterates the cycle from 0 to x (x=number of sentences). import sentences as a list. sentences are in one file. how many objects in the list? iterate for the number of objects
	...
	for j in range (len (sentence)): #number of words (sentence vs. sentences)
		...
		#######################################################
		#manual check on fortasse: problem with lemmatisation, so I check the form
		#######################################################
		if (modal_lemmas_1.count(token["form"]) != 0):
			if (token["form"]) == 'fortasse':
				cooccurrence = cooccurrence + 1
				list_lemmas_in_sentence = list_lemmas_in_sentence + token["form"]+','
...
#		print (token["form"])
		...
		if (token["lemma"]) == 'aequus': #token["lemma"] is elaborated here
			cooccurrence = cooccurrence + 1 #it can be the adverb aeque, which can be modal: increase the counter even if not followed by auxiliary verb
			
			if (j == (len(sentence)-1)):
				list_lemmas_in_sentence = list_lemmas_in_sentence + token["lemma"] + ','
			
			elif (token["form"]) == 'aequum' and (token_2["lemma"]) == 'sum':
				list_lemmas_in_sentence = list_lemmas_in_sentence + token["form"] + ' ' + token_2["lemma"] + ',' #this passage does not work
			else:
				list_lemmas_in_sentence = list_lemmas_in_sentence + token["lemma"] + ',' #do it even if it is not the last token

				

	#if there is one modal marker in the sentence
...
f = open(name_file_no_extension + '.one_marker', 'w') #open file in write 'w' (for reading 'r')
...
# f = open(name_file_no_extension + '_totmarkers' + '.conllu', 'w')
# #string_output_sentences = str(output_sentences)
...
# #####################################################################
# #
# #   calculate percentage of co-occurrences on total number of sentences
# #
# ####################################

# perc_0 = (n_sentences_0l / len(sentences)) * 100
# perc_1 = (n_sentences_1l / len(sentences)) * 100
# perc_2 = (n_sentences_2l / len(sentences)) * 100
# perc_3 = (n_sentences_3l / len(sentences)) * 100
# perc_2ormore = (n_output_sentences / len(sentences)) * 100
...
# #
# ######################################################################
# dict = {'Total number of sentences:                    ' : [len(sentences), '100'], 'Sentences with 0 markers:        ' : [n_sentences_0l, perc_0], 'Sentences with 1 marker:        ' : [n_sentences_1l, perc_1],\
#   'Sentences with 2 markers:        ' : [n_sentences_2l, perc_2], 'Sentences with 3 or more markers: ' : [n_sentences_3l, perc_3], 'Sentences with at least 2 co-occurring markers: ' : [n_output_sentences, perc_2ormore]}
# w = csv.writer(open(name_file_no_extension+'_statistics'+'.csv', 'w')) #w is a name (as f)
# for key, value in dict.items(): 
# 	w.writerow([key, *value]) #write the row with key + value
...
print ('# Sentences with at least 1 marker: ', n_output_sentences)
# print ('#')
...
print ('#')
print ('# Output file written on ' + name_file_no_extension + '.one marker')
print ('#')
print ('########################################################')
